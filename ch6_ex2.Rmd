...
Is the effect of election statistically significant under democrats? - filter for democrats
```{r}
olsdemocrat <- lm(dem_plot$FEDFUNDS ~ dem_plot$election)
summary(olsdemocrat)
```

2d. Fitted lines for relationship between election and interest rates
```{r}
#View(dta)
plot(dem_plot$election, dem_plot$FEDFUNDS, main="Quarters and Federal Funds Rate",
     xlab="Quarters Since Previous Election", ylab = "FFR (%)", pch=19, col='blue')
points(rep_plot$election, rep_plot$FEDFUNDS, pch=18, col='red')
abline(lm(dem_plot$FEDFUNDS ~ dem_plot$election), col = 'blue')
abline(lm(rep_plot$FEDFUNDS ~ rep_plot$election), col = 'red')
